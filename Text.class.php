  ...
  /**
	...
	public function VideoParser($sText) {
		...
		return $sText;
		 ...
		 /**
	 * Парсинг текста на предмет custom tags
	 * Находит теги <pre><zoomit></zoomit></pre> и преобразует их
	 * Находит теги <pre><issuu></issuu></pre> и преобразует их
	 * @param string $sText	Исходный текст
	 * @return string
	 */
	public function CustomTagParser($sText) {
	/**
		 * custom tags handle: <zoomit>
		 * syntax: <zoomit>[id]</zoomit> or <zoomit>http://[www.]zoom.it/id</zoomit> 

	*/
		$sText = preg_replace('/<zoomit>http:\/\/(?:www\.|)zoom\.it\/([A-Z,a-z]+).*<\/zoomit>|<zoomit>\[([A-Z,a-z]+)\]<\/zoomit>/Ui', 
			'<script src="http://zoom.it/$1$2.js?width=auto&height=400px"></script>', 
		$sText);
		
		$sText = preg_replace('/<zoomit>.*<\/zoomit>/i', '&lt;zoomit&gt; tag syntax error', $sText);

	/**
		 * custom tags handle: <issuu>
		 * syntax: <issuu>[id]</issuu> or <zoomit>http://[www.]issuu.com/sovdoma/docs/chtk_entuz_izmail_komplekt/3?e=8392905/2825104</zoomit> 

	
		$sText = preg_replace('/<issuu>http:\/\/(?:www\.|)issuu\.com\/[a-zA-Z0-9_\-]+\?e=(\d+\/\d+).*<\/issuu>|<issuu>\[(\d+\/\d+)\]<\/issuu>/Ui', 
			'<iframe width="635" height="411" src="http://e.issuu.com/embed.html#$1$2" frameborder="0" allowfullscreen></iframe>', 
		$sText);
		* Embedding errors sometimes happens with above code
	*/
	
		$sText = preg_replace('/<issuu>http:\/\/(?:www\.|)issuu\.com\/[a-zA-Z0-9_\-]+\?e=(\d+\/\d+).*<\/issuu>|<issuu>\[(\d+\/\d+)\]<\/issuu>/Ui', 
			'<div data-configid="$1$2" style="width: 635px; height: 411px;" class="issuuembed"></div><script type="text/javascript" src="http://e.issuu.com/embed.js" async="true"></script>', 
		$sText);
		
		$sText = preg_replace('/<issuu>.*<\/issuu>/i', '&lt;issuu&gt; tag syntax error', $sText);
	...
	public function Parser($sText) {
		...
		$sResult=$this->CustomTagParser($sResult);
